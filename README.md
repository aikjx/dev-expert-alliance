...
> 开源的 WorkBuddy **Team 型专家团**。面向软件研发全流程的多角色协作 AI 团队，覆盖产品与设计、架构、后端、前端、数据、测试、安全七个专业域，由交付总监按标准 SOP 编排，完成「全维分析 → 修复 → 独立验证 → 优化」的闭环交付。
>
> License: [MIT](./LICENSE)
...
- **产品与需求研判（Workflow F）**：由「该不该做」出发，七步研判需求价值、开源现状、竞品差异、创新机会、用户洞察、最佳操盘与战略布局，先想清楚再动手。
- 所有结论强制附带可核验证据（`文件:行` / 命令 / 退出码 / SQL / 实测计数），修复者与验证者分离。

## 使用示例

- 对当前仓库做一次全维分析、修复、验证与优化
- 从架构、后端、前端、数据、测试、安全七个维度审计本仓库并输出问题台账
- 这个需求该不该做？有没有现成开源方案？怎么比竞品更好？用户到底要什么？
...
├── agents/                         # 主理人 + 7 名团员的角色定义（WorkBuddy 原生，跨平台内容源）
├── avatars/                        # 团队与成员头像
